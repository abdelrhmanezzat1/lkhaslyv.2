...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method !== 'POST') {
      return new Response(
        JSON.stringify({ error: 'Method not allowed. Use POST with notification payload.' }),
    ...
    if (tokens.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: 'No valid tokens found', processed: 0, failed: 0 }),
...
async function sendToTokensWithRetry(tokens: string[], payload: NotificationPayload): Promise<ListenerResult> {
  ...
  for (let i = 0; i < messages.length; i += batchSize) {
    ...
    const responses = await Promise.allSettled(
      batch.map((msg, idx) => sendWithRetry(fcmUrl, accessToken, msg, batchTokens[idx], 3))
    )
    ...
    responses.forEach((response, index) => {
      if (response.status === 'fulfilled') {
        ...
        failed++
        const errorMsg = `Token ${batchTokens[index]}: ${response.reason?.message || 'Unknown error'}`
...
async function sendWithRetry(url: string, accessToken: string, message: object, token: string, maxRetries: number): Promise<void> {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (response.status === 400 || response.status === 404) {
        const error = data.error?.message || `FCM error: ${response.status}`
        if (error.includes('UNREGISTERED') || error.includes('INVALID_ARGUMENT') || error.includes('NOT_FOUND')) {
          console.warn(`Token invalid/unregistered, skipping retry: ${token}`)
...
async function sendToTopic(payload: NotificationPayload): Promise<Response> {
  ...
  return new Response(
    JSON.stringify({ success: true, processed: 1, failed: 0, messageId: data.name }),
    { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
  )
}
